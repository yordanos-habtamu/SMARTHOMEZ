...
func (h *Handler) handleRegisterhouse(w http.ResponseWriter, r *http.Request) {
	...
	numBathrooms, err := strconv.Atoi(r.FormValue("numBathrooms")); if err!=nil{
		log.Printf("error parsing %v",err)
		utils.WriteError(w, http.StatusBadRequest, fmt.Errorf("failed to parse the form %v", err))
		return
	}
	numBedrooms, err := strconv.Atoi(r.FormValue("numBedRooms"));if err!=nil{
		log.Printf("error parsing %v",err)
		utils.WriteError(w, http.StatusBadRequest, fmt.Errorf("failed to parse the form %v", err))
		return
	}
	price,err:= strconv.ParseFloat(r.FormValue("price"), 64);if err!=nil{
		log.Printf("error parsing %v",err)
		utils.WriteError(w, http.StatusBadRequest, fmt.Errorf("failed to parse the form %v", err))
		return
	}
	agentID, err := strconv.ParseUint(r.FormValue("agentId"), 10, 32);if err!=nil{
		log.Printf("error parsing %v",err)
		utils.WriteError(w, http.StatusBadRequest, fmt.Errorf("failed to parse the form %v", err))
		return
	}
	areaSqFt, err:= strconv.Atoi(r.FormValue("areaSqFt"));if err!=nil{
		log.Printf("error parsing %v",err)
		utils.WriteError(w, http.StatusBadRequest, fmt.Errorf("failed to parse the form %v", err))
		return
	}
	isSold, err:= strconv.ParseBool(r.FormValue("isSold"));if err!=nil{
		log.Printf("error parsing %v",err)
		utils.WriteError(w, http.StatusBadRequest, fmt.Errorf("failed to parse the form %v", err))
		return
	}
